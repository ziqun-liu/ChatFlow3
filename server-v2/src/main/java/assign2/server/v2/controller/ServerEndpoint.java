...
public class ServerEndpoint {
    ...
    private static final String SERVER_ID = System.getenv("SERVER_ID") != null ? System.getenv("SERVER_ID") : "server-1";
    ...
    public void onMessage(String message, Session session, @PathParam("roomId") String roomId) {
        ...
        if (!hasJoined && ("TEXT".equals(messageType) || "LEAVE".equals(messageType))) {
            sendError(session, chatMsg.getMessageId(), "Must JOIN the room before sending " + messageType);
            return;
        }
        ...
        UserManager.register(chatMsg.getUserId(), new UserInfo(chatMsg.getUserId(), chatMsg.getUsername(), roomId, session, System.currentTimeMillis()));

        // 4. Publish to RabbitMQ (with publisher confirms)
        String clientIp = session.getUserProperties().getOrDefault("javax.websocket.endpoint.remoteAddress", "unknown").toString();
        ...
        if (confirmed) {
            logger.fine("roomId=" + roomId + ",messageType=" + messageType + ",messageId=" + chatMsg.getMessageId() + " gets broadcasted");
    ...
    public void onClose(Session session, @PathParam("roomId") String roomId, CloseReason reason) {
        ...
        logger.fine("Session closed: session=" + session.getId() + ", room=" + roomId + ", reason=" + reason);
    }

    @OnError
    public void onError(Session session, Throwable exception) {
        logger.severe("Transport error: session=" + session.getId() + ", error=" + exception.getMessage());
    }
    ...
    private void sendError(Session session, String messageId, String errorMsg) {
        synchronized (session) {
            try {
                ChatResponse errResp = messageId != null ? ChatResponse.error(messageId, errorMsg) : ChatResponse.error(errorMsg);
