...
export CATALINA_OPTS="\
  ...
  -DRABBITMQ_CHANNEL_POOL_SIZE=$RABBITMQ_CHANNEL_POOL_SIZE \
  -DRABBITMQ_DELIVERY_MODE=$RABBITMQ_DELIVERY_MODE \
  -DDB_HOST=$DB_HOST \
  -DDB_PORT=${DB_PORT:-3306} \
  -DDB_NAME=${DB_NAME:-chatflow} \
  -DDB_USER=$DB_USER \
  -DDB_PASS=$DB_PASS \
  -DDB_POOL_SIZE=${DB_POOL_SIZE:-5}"
ENV
